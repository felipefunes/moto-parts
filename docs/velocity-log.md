...
| 2026-09-13 | Rebrand `motos` theme to Worth Moto Parts | — (no estimate given) | not meaningful | No clean anchor: this is a tangential, user-directed task (a real brand name + domain + draft logo arrived) done in parallel with PR #28 (fast E2E spec) still sitting open for review, not sequentially after it. Renamed the site everywhere it appears (siteName, logo wordmark, both in-house brand strings in the product seed, the SKU prefix, the theme color tokens to match the logo's red-orange gradient, `render.yaml`'s domain, docs). The actual badge artwork (piston + gear) couldn't be added as a file: the only copy was a macOS screenshot in a temp path that had already been cleaned up by the time it would've been copied, so `Logo.tsx`'s SVG mark was recolored in place as a placeholder rather than replaced. |
